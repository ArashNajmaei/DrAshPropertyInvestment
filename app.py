...
# ===========================================================================
def report_html(x, results, score, metrics):
    h = metrics["horizon"]
    first_year = results["cashflow"].iloc[0]
    ...
    fields = {
        ...
        "Tax basis": tax_basis, "Feasibility score": f"{score:.0f}/100 ({verdict})",
...
# ===========================================================================
verdict = "Strong" if score >= 75 else "Viable" if score >= 55 else "Weak"
verdict_class = "is-strong" if score >= 75 else "is-viable" if score >= 55 else "is-weak"
...
st.markdown(
    ...
    <div class="hero {verdict_class}">
        <div class="hero-copy">
            ...
            sensitivity analysis and an opportunity-cost benchmark.</p>
            <div class="hero-byline">Analytical property intelligence · <b>Dr Ash Najmaei</b></div>
        </div>
        <div class="hero-score" style="--val:{score:.0f}">
...
tab_dash, tab_loan, tab_cash, tab_growth, tab_risk, tab_sens, tab_scen, tab_export = st.tabs([
    "Executive dashboard", "Purchase & loan", "Cash flow & tax", "Growth, equity & returns",
    "Risk & Monte Carlo", "Sensitivity & break-even", "Scenarios", "Export",
])
...
with tab_dash:
    ...
    if score >= 75:
        st.markdown(f'<div class="good">Feasibility score <b>{score:.0f}/100 — Strong</b>. The deal is well '
        ...
        st.markdown(f'<div class="warn">Feasibility score <b>{score:.0f}/100 — Viable</b>. The deal stacks up '
                    f'but has sensitivities worth stress-testing in the Risk and Sensitivity tabs.</div>',
                    unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="bad">Feasibility score <b>{score:.0f}/100 — Weak</b> on current assumptions. '
                    f'Review LVR, yield, cash flow and growth before proceeding.</div>', unsafe_allow_html=True)

    with st.expander("How the feasibility score is composed"):
...
with tab_risk:
    ...
    with cset1:
        ...
        cg_sd = st.slider("Capital growth volatility (± p.a.)", 0.0, 0.08, 0.02, 0.005, format="%.3f")
...
with tab_scen:
    ...
    def snapshot(name, x, results, metrics, score):
        ...
        return {
            "inputs": asdict(x), "Score": round(score, 1), "Final LVR": results["final_lvr"],
    ...
    if scenarios:
        comp = pd.DataFrame({
            "Metric": ["Feasibility score", "Final LVR", "Gross yield", "Cash required",
                       "Year 1 after-tax CF", "IRR (after-tax)", "NPV @ benchmark", "Total profit", "Horizon"],
        })
        for name, s in scenarios.items():
            comp[name] = [f"{s['Score']:.0f}/100", pct(s["Final LVR"]), pct(s["Gross yield"]),
        ...
        if len(scenarios) >= 2:
            ...
            fig.add_trace(go.Bar(x=names, y=[scenarios[n]["Score"] for n in names],
                                 name="Feasibility score", marker_color=BRASS,
                                 yaxis="y2", offsetgroup=2))
            fig.update_layout(title="Profit and feasibility across scenarios", height=380, barmode="group",
...
with tab_export:
    ...
    report = report_html(inputs, results, score, metrics)
    ...
    with c2:
        ...
        summary = pd.DataFrame([{
            "Property": property_name, "Score": round(score, 1), "Verdict": verdict,
            "Final LVR": results["final_lvr"], "Cash required": results["cash_required"],
