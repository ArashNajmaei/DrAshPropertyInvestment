...
# ===========================================================================
# Breakeven sale price — the sale price at which total profit is exactly $0,
# consistent with the engine (loan, holding cash flows, selling costs, CGT).
# ===========================================================================
def breakeven_sale_price(x, results):
    h = min(x.horizon, len(results["growth"]))
    grow = results["growth"].iloc[h - 1]
    loan_balance = float(grow["Loan Balance"])
    cum_cashflow = float(grow["Cumulative After-tax Cash Flow"])
    cash_required = results["cash_required"]
    cost_base = results["cost_base"]
    scp = x.selling_cost_pct

    def profit(sale):
        selling = sale * scp
        gain = max(0.0, sale - selling - cost_base)
        cgt = capital_gains_tax(x, gain, h)
        net_sale = sale - selling - loan_balance - cgt
        return net_sale + cum_cashflow - cash_required

    hi = max(cost_base * 6, x.purchase_price * 6, 2_000_000.0)
    be = _bisect(profit, 0.0, hi, 0.0)
    if be is None:
        be = cost_base
    selling = be * scp
    gain = max(0.0, be - selling - cost_base)
    cgt = capital_gains_tax(x, gain, h)
    # residual net holding costs (excludes principal — that is recovered at sale)
    net_holding = be - x.purchase_price - results["purchase_costs"] - selling - cgt
    projected_value = float(grow["Property Value"])
    margin = projected_value - be
    implied_growth = ((be / x.purchase_price) ** (1.0 / h) - 1.0) if (x.purchase_price > 0 and be > 0 and h > 0) else float("nan")
    return {
        "horizon": h, "breakeven": be, "projected_value": projected_value,
        "margin": margin, "margin_pct": (margin / be if be else float("nan")),
        "implied_growth": implied_growth,
        "components": {
            "Purchase price": x.purchase_price, "Upfront costs": results["purchase_costs"],
            "Net holding costs": net_holding, "Selling costs": selling, "Capital gains tax": cgt,
        },
        "cumulative_cashflow": cum_cashflow, "loan_balance": loan_balance,
    }
...
# ===========================================================================
def report_html(x, results, score, metrics, adv, notes="", floor_plan_uri=""):
    h = metrics["horizon"]
    first_year = results["cashflow"].iloc[0]
    ...
    be = breakeven_sale_price(x, results)
    ...
    fields = {
        ...
        f"Breakeven sale price (yr {h})": money(be["breakeven"]),
        "Growth needed to break even": f'{pct(be["implied_growth"])} p.a. (assumed {pct(x.capital_growth)})',
        "Margin (value − breakeven)": f'{money(be["margin"])} ({be["margin_pct"]:+.1%})',
    ...
    if notes and notes.strip():
        ...
        notes_html += f'<div class="userblock"><h3>Analyst notes</h3><p class="note-body">{safe}</p></div>'
    if floor_plan_uri:
        notes_html += (f'<div class="userblock"><h3>Floor plan</h3>'
                       f'<img class="floorplan" src="{floor_plan_uri}" alt="Floor plan"></div>')
    ...
    <style>
        ...
        * {{ box-sizing:border-box; }}
        ...
        .userblock {{ padding:8px 38px 6px; }}
        .userblock h3 {{ font-family:'Fraunces',Georgia,serif; font-size:1rem; font-weight:600; color:var(--ink);
            margin:14px 0 8px; padding-bottom:5px; border-bottom:1px solid var(--line); }}
        .note-body {{ font-size:0.9rem; color:#1A2A3A; line-height:1.55; white-space:pre-wrap;
            background:#F5F7FA; border-left:4px solid var(--brass); border-radius:8px; padding:12px 14px; }}
        .floorplan {{ width:100%; border:1px solid var(--line); border-radius:12px; }}
    </style></head>
    <body><div class="report">
        <div class="report-head">
            ...
            <div class="badge"><div class="num">{composite:.0f}</div><div class="verdict">{verdict}</div></div>
        </div>
...
with st.sidebar:
    st.subheader("Notes & floor plan")
    property_notes = st.text_area("Notes on this property", height=130,
        placeholder="Condition, location, comparables, risks, negotiation points, follow-ups…")
    floor_plan_file = st.file_uploader("Floor plan image", type=["png", "jpg", "jpeg", "webp", "gif"],
        help="Shown on the Executive dashboard, embedded in the exported report, and saved with the scenario.")

floor_plan_uri = ""
if floor_plan_file is not None:
    _mime = floor_plan_file.type or "image/png"
    floor_plan_uri = "data:" + _mime + ";base64," + base64.b64encode(floor_plan_file.getvalue()).decode()
...
with tab_dash:
    ...
    if property_notes.strip() or floor_plan_uri:
        st.markdown("### Property notes & floor plan")
        nc1, nc2 = st.columns([1, 1])
        with nc1:
            if property_notes.strip():
                st.markdown(f'<div class="good" style="white-space:pre-wrap">{property_notes}</div>',
                            unsafe_allow_html=True)
            else:
                st.caption("No notes entered — add them in the sidebar.")
        with nc2:
            if floor_plan_uri:
                st.markdown(f'<img src="{floor_plan_uri}" style="width:100%;border:1px solid var(--line);'
                            f'border-radius:12px" alt="Floor plan">', unsafe_allow_html=True)
            else:
                st.caption("No floor plan uploaded — add one in the sidebar.")
...
with tab_growth:
    ...
    st.markdown("### Breakeven sale price")
    st.markdown('<p class="lead">The sale price at which you walk away exactly even after recovering the purchase '
                'price, upfront costs, net holding costs, selling costs and CGT. It reframes "what might it be '
                'worth" into "what does it need to be worth".</p>', unsafe_allow_html=True)
    be = breakeven_sale_price(inputs, results)
    comp = be["components"]

    bm1, bm2, bm3 = st.columns(3)
    bm1.metric("Breakeven sale price", money(be["breakeven"]),
               help="Sale price at the horizon that yields exactly $0 total profit.")
    bm2.metric(f"Projected value (yr {be['horizon']})", money(be["projected_value"]))
    bm3.metric("Margin of safety", money(be["margin"]), f"{be['margin_pct']:+.1%}",
               delta_color="normal", help="Projected value minus breakeven price.")

    _ig, _ag = be["implied_growth"], inputs.capital_growth
    if _ig <= _ag:
        st.markdown(f'<div class="good">You need <b>{pct(_ig)} p.a.</b> capital growth just to break even — '
                    f'below your assumed <b>{pct(_ag)} p.a.</b>, so the assumption clears the hurdle with room '
                    f'to spare. (This is the break-even growth shown in the Sensitivity tab.)</div>',
                    unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="warn">You need <b>{pct(_ig)} p.a.</b> capital growth just to break even — '
                    f'above your assumed <b>{pct(_ag)} p.a.</b>. The deal needs growth stronger than you are '
                    f'projecting to avoid a loss at sale. (This is the break-even growth shown in the Sensitivity '
                    f'tab.)</div>', unsafe_allow_html=True)

    labels = ["Purchase price", "Upfront costs", "Net holding costs", "Selling costs",
              "Capital gains tax", "Breakeven price"]
    fig = go.Figure(go.Waterfall(
        orientation="v",
        measure=["absolute", "relative", "relative", "relative", "relative", "total"],
        x=labels,
        y=[comp["Purchase price"], comp["Upfront costs"], comp["Net holding costs"],
           comp["Selling costs"], comp["Capital gains tax"], 0],
        text=[money(comp["Purchase price"]), money(comp["Upfront costs"]), money(comp["Net holding costs"]),
              money(comp["Selling costs"]), money(comp["Capital gains tax"]), money(be["breakeven"])],
        textposition="outside", connector=dict(line=dict(color=SLATE, width=1)),
        increasing=dict(marker=dict(color=BRASS)), decreasing=dict(marker=dict(color=POS)),
        totals=dict(marker=dict(color=INK))))
    fig.update_layout(title="How the breakeven price is built up", height=380,
                      yaxis_tickprefix="$", yaxis_tickformat=",.0f", showlegend=False)
    st.plotly_chart(fig, use_container_width=True)

    if be["margin"] >= 0:
        st.markdown(f'<div class="good">Your projected value of <b>{money(be["projected_value"])}</b> sits '
                    f'<b>{money(be["margin"])}</b> ({be["margin_pct"]:+.1%}) above the breakeven of '
                    f'<b>{money(be["breakeven"])}</b> — a cushion against weaker growth or higher costs.</div>',
                    unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="bad">Your projected value of <b>{money(be["projected_value"])}</b> is '
                    f'<b>{money(-be["margin"])}</b> ({be["margin_pct"]:+.1%}) <b>below</b> the breakeven of '
                    f'<b>{money(be["breakeven"])}</b> — on these assumptions the deal loses money at sale. It '
                    f'relies on stronger growth, higher rent or lower costs.</div>', unsafe_allow_html=True)

    be_tbl = pd.DataFrame({"Component": list(comp.keys()), "Amount": [money(v) for v in comp.values()]})
    be_tbl.loc[len(be_tbl)] = ["= Breakeven sale price", money(be["breakeven"])]
    st.dataframe(be_tbl, use_container_width=True, hide_index=True)
    st.caption("Net holding costs exclude loan principal, which is recovered at sale rather than being a true "
               "cost — so this breakeven is typically lower than sites that treat every dollar paid as a cost.")
...
with tab_scen:
    ...
    def snapshot(name, x, results, metrics, score, notes="", floor_plan=""):
        h = metrics["horizon"]
        ...
        return {
            ...
            "Notes": notes, "Floor plan": floor_plan,
            "Pillars": {k: (round(v["score"], 1) if v["score"] is not None else None)
                        for k, v in adv_s["pillars"].items()},
        }
    ...
    with csave2:
        ...
        if st.button("💾 Save current scenario", use_container_width=True):
            st.session_state["scenarios"][scen_name] = snapshot(
                scen_name, inputs, results, metrics, score, property_notes, floor_plan_uri)
            st.success(f"Saved '{scen_name}' with its notes and floor plan.")
    ...
    if scenarios:
        ...
        # ---- Two-property side-by-side (A vs B) ----
        st.markdown("### Two-property side-by-side (A vs B)")
        if len(scenarios) >= 2:
            def esc(s):
            ...
            pa, pb = st.columns(2)
            with pa:
                sel_a = st.selectbox("Property A", names, index=0, key="cmp_a")
            with pb:
                sel_b = st.selectbox("Property B", names, index=min(1, len(names) - 1), key="cmp_b")
            A, B = scenarios[sel_a], scenarios[sel_b]

            fa, fb = st.columns(2)
            for col, nm, S in [(fa, sel_a, A), (fb, sel_b, B)]:
                with col:
                    st.markdown(f"**{esc(nm)}**")
                    if S.get("Floor plan"):
                        st.markdown(f'<img src="{S["Floor plan"]}" style="width:100%;border:1px solid var(--line);'
                                    f'border-radius:12px" alt="Floor plan">', unsafe_allow_html=True)
                    else:
                        st.caption("No floor plan saved with this scenario.")

            metric_rows = [
                ("Composite score", "Score", "high", lambda v: f"{v:.0f}/100"),
                ("Financial score", "Financial", "high", lambda v: f"{v:.0f}/100"),
                ("IRR (after-tax)", "IRR", "high", pct1),
                ("NPV @ benchmark", "NPV", "high", money),
                ("Total profit", "Total profit", "high", money),
                ("Gross yield", "Gross yield", "high", pct),
                ("Year 1 after-tax CF", "Year 1 after-tax CF", "high", money),
                ("Final LVR", "Final LVR", "low", pct),
                ("Cash required", "Cash required", "low", money),
            ]

            def _cell(val, win, fmt):
                sty = "background:rgba(31,122,92,0.14);font-weight:700;" if win else ""
                return f'<td style="padding:8px 12px;border-bottom:1px solid #E4EAF1;{sty}">{fmt(val)}</td>'

            def _num(v):
                return v is not None and not (isinstance(v, float) and math.isnan(v))

            html = ['<table style="width:100%;border-collapse:collapse;font-size:0.9rem;margin-top:6px">',
                    f'<tr><th style="text-align:left;padding:8px 12px;color:#55677B;font-size:0.74rem;'
                    f'text-transform:uppercase">Metric</th>'
                    f'<th style="text-align:left;padding:8px 12px;color:#0F2138">{esc(sel_a)}</th>'
                    f'<th style="text-align:left;padding:8px 12px;color:#0F2138">{esc(sel_b)}</th>'
                    f'<th style="text-align:left;padding:8px 12px;color:#55677B;font-size:0.74rem;'
                    f'text-transform:uppercase">Better</th></tr>']
            a_wins = b_wins = 0
            for label, key, better, fmt in metric_rows:
                va, vb = A.get(key), B.get(key)
                if not (_num(va) and _num(vb)):
                    wa = wb = False
                    win = "—"
                elif abs(va - vb) < 1e-9:
                    wa = wb = False
                    win = "Tie"
                elif (va > vb) == (better == "high"):
                    wa, wb, win = True, False, "A"
                    a_wins += 1
                else:
                    wa, wb, win = False, True, "B"
                    b_wins += 1
                html.append('<tr><td style="padding:8px 12px;border-bottom:1px solid #E4EAF1;color:#0F2138;'
                            f'font-weight:600">{label}</td>' + _cell(va, wa, fmt) + _cell(vb, wb, fmt) +
                            f'<td style="padding:8px 12px;border-bottom:1px solid #E4EAF1;font-weight:700;'
                            f'color:#C29B4A">{win}</td></tr>')
            html.append("</table>")
            st.markdown("".join(html), unsafe_allow_html=True)

            if a_wins > b_wins:
                st.markdown(f'<div class="good">On these metrics <b>{esc(sel_a)}</b> leads {a_wins}–{b_wins}. '
                            f'Weigh it against the notes and any qualitative factors.</div>', unsafe_allow_html=True)
            elif b_wins > a_wins:
                st.markdown(f'<div class="good">On these metrics <b>{esc(sel_b)}</b> leads {b_wins}–{a_wins}. '
                            f'Weigh it against the notes and any qualitative factors.</div>', unsafe_allow_html=True)
            else:
                st.markdown(f'<div class="warn">The two are evenly matched ({a_wins}–{b_wins}) — the decision rests '
                            f'on notes and qualitative factors.</div>', unsafe_allow_html=True)

            pa_sc, pb_sc = A.get("Pillars", {}) or {}, B.get("Pillars", {}) or {}
            cats = [k for k in pa_sc if pa_sc.get(k) is not None and pb_sc.get(k) is not None]
            if cats:
                fig = go.Figure()
                fig.add_trace(go.Bar(y=cats, x=[pa_sc[k] for k in cats], name=sel_a, orientation="h", marker_color=INK))
                fig.add_trace(go.Bar(y=cats, x=[pb_sc[k] for k in cats], name=sel_b, orientation="h", marker_color=BRASS))
                fig.update_layout(title="Pillar scores — A vs B", height=340, barmode="group",
                                  xaxis=dict(range=[0, 100], title="Score"))
                st.plotly_chart(fig, use_container_width=True)

            na, nb = st.columns(2)
            with na:
                st.markdown(f"**{esc(sel_a)} — notes**")
                st.markdown(f'<div class="good" style="white-space:pre-wrap">{esc(A.get("Notes") or "—")}</div>',
                            unsafe_allow_html=True)
            with nb:
                st.markdown(f"**{esc(sel_b)} — notes**")
                st.markdown(f'<div class="good" style="white-space:pre-wrap">{esc(B.get("Notes") or "—")}</div>',
                            unsafe_allow_html=True)
        else:
            st.caption("Save at least two scenarios to unlock the A-vs-B comparison.")
        ...
        with cman1:
            ...
            if st.button("Delete selected", use_container_width=True) and to_delete != "—":
                ...
                st.rerun()
        with cman2:
            payload = {n: {"inputs": s["inputs"], "notes": s.get("Notes", ""),
                           "floor_plan": s.get("Floor plan", "")} for n, s in scenarios.items()}
            st.download_button("⬇ Download scenarios (JSON, incl. notes & floor plans)",
        ...
        try:
            ...
            for name, entry in data.items():
                if isinstance(entry, dict) and "inputs" in entry:
                    ...
                    floor = entry.get("floor_plan", "")
                else:  # legacy format: entry is the inputs dict itself
                    inp, notes, floor = entry, "", ""
                xi = Inputs(**inp)
                ri = calculate(xi)
                mi = return_metrics(xi, ri)
                si, _ = investment_score(xi, ri)
                st.session_state["scenarios"][name] = snapshot(name, xi, ri, mi, si, notes, floor)
                restored += 1
...
with tab_export:
    ...
    report = report_html(inputs, results, score, metrics, adv,
                         notes=property_notes, floor_plan_uri=floor_plan_uri)
