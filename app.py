...
def report_html(x,r):
    ...
    fields={"Property":x.property_name,"Property type":x.property_type,"Strata applies":"Yes" if x.strata_applies else "No","State":x.state,"Purchase price":money(x.purchase_price),"Deposit":money(x.deposit),"Base LVR":pct(r["base_lvr"]),"LMI added":money(r["lmi_used"]),"Total loan":money(r["total_loan"]),"Duty used":money(r["duty_used"]),"Total cash required":money(r["cash_required"]),"Year 1 pre-tax cash flow":money(c["Pre-tax Cash Flow"]),"Year 1 after-tax cash flow":money(c["After-tax Cash Flow"]),"Weekly out-of-pocket":money2(short/52),"Fortnightly out-of-pocket":money2(short/26),"Monthly out-of-pocket":money2(short/12),f"Value after {h} years":money(g["Property Value"]),f"Equity after {h} years":money(g["Gross Equity"]),f"Total profit after {h} years":money(g["Total Profit"])}
...
st.title("Property Investment Decision Dashboard by Dr Arash (Ash)")
